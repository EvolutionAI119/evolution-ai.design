        ...
        <el-card class="config-card">
          ...
          <el-form :model="exportForm" label-width="100px">
            ...
            <el-form-item label="精度设置">
              ...
              <span class="precision-hint">默认 0.01mm</span>
            </el-form-item>

            <el-form-item label="元数据">
              <el-checkbox v-model="exportForm.include_metadata">包含元数据</el-checkbox>
            </el-form-item>
          </el-form>
          ...
          <div v-else class="history-list">
            ...
            >
              <div class="history-header">
                <span class="history-title">模型 {{ history.model_id }}</span>
              ...
              <div class="history-actions">
                <el-button type="text" size="small" @click="downloadFile(history.model_id, history.formats[0])">下载</el-button>
              </div>
            </div>
          </div>
        </el-card>
        ...
        <el-card class="result-card" v-if="exportResult">
          ...
          <div class="result-summary">
            <div class="summary-item">
              <span class="summary-label">模型ID</span>
              <span class="summary-value">{{ exportResult.model_id }}</span>
            </div>
            <div class="summary-item">
              <span class="summary-label">导出耗时</span>
        ...
        <el-card class="formats-info">
          ...
          <div class="format-descriptions">
            <div class="format-desc-item">
              ...
              <span class="desc-text">适用于 Web 3D 展示和实时渲染</span>
            </div>
            <div class="format-desc-item">
              <span class="desc-format">STL</span>
              <span class="desc-text">3D 打印标准格式，三角形网格</span>
            </div>
            <div class="format-desc-item">
              <span class="desc-format">OBJ</span>
              <span class="desc-text">通用多边形格式，支持材质</span>
            </div>
            <div class="format-desc-item">
              <span class="desc-format">STEP/IGES</span>
              <span class="desc-text">CAD 工程交换格式，保留曲面信息</span>
            </div>
            <div class="format-desc-item">
              <span class="desc-format">JSON</span>
              <span class="desc-text">参数化数据格式，便于二次开发</span>
            </div>
          </div>
        </el-card>
      </div>
...
import { Download, File, Image, Box, FileJson, Cpu, FileText, Database } from '@element-plus/icons-vue'
...
const availableFormats = [
  ...
  { key: 'stl', name: 'STL', description: '3D打印', icon: Image, color: '#ff6b6b' },
  { key: 'obj', name: 'OBJ', description: '多边形', icon: Image, color: '#ffd93d' },
  { key: 'step', name: 'STEP', description: 'CAD交换', icon: FileText, color: '#6bcb77' },
  { key: 'iges', name: 'IGES', description: 'IGES', icon: FileText, color: '#4d96ff' },
  { key: 'json', name: 'JSON', description: '数据格式', icon: FileJson, color: '#9966ff' }
]
...
.format-name { font-weight: bold; font-size: 14px; }
.format-desc { font-size: 11px; color: #999; margin-top: 3px; }

.precision-hint { margin-left: 10px; color: #999; font-size: 12px; }
...
.exported-files h4 { margin-bottom: 15px; }
...
.format-descriptions { display: flex; flex-direction: column; gap: 10px; }
...
.desc-text { color: #666; font-size: 14px; }
</style>
