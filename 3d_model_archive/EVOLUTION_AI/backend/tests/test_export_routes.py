...
from app.models.database import init_db, SessionLocal, ModelFile
...
# ============ 补充：源文件存在时走 DataHandover ============

class TestExportWithSourceFile:
    def _setup_source_file(self, model_id):
        """辅助：为模型创建真实源文件，使 Path.exists 自然返回 True"""
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as tmp:
            tmp.write(b'{"test": true}')
            tmp_path = tmp.name
        db = SessionLocal()
        model = db.query(ModelFile).filter(ModelFile.id == model_id).first()
        model.filepath = tmp_path
        db.commit()
        db.close()
        return tmp_path

    def _cleanup_source_file(self, tmp_path):
        """辅助：清理临时源文件"""
        try:
            os.unlink(tmp_path)
        except OSError:
    ...
    def test_export_stl_source_file_exists(self):
        """源文件存在时，STL 导出走 DataHandover 路径"""
        model_id = _create_project_and_model()
        tmp_path = self._setup_source_file(model_id)

        try:
            with patch("app.api.export_routes.DataHandover") as MockHandover:
                MockHandover.return_value.prepare_handover.return_value = None
                resp = client.post("/api/v1/export/", json={
                    "model_id": model_id,
                    "formats": ["stl"],
                })
                assert resp.status_code == 200
                MockHandover.assert_called_once()
        finally:
            self._cleanup_source_file(tmp_path)

    def test_export_step_source_file_exists(self):
        """源文件存在时，STEP 导出走 DataHandover 路径"""
        model_id = _create_project_and_model()
        tmp_path = self._setup_source_file(model_id)
    ...
    def test_export_obj_source_file_exists(self):
        """源文件存在时，OBJ 导出走 DataHandover 路径"""
        model_id = _create_project_and_model()
        tmp_path = self._setup_source_file(model_id)
...
class TestExportExceptions:
    def test_export_500_on_error(self):
        """导出过程抛异常 → 500"""
        model_id = _create_project_and_model()
        with patch("app.api.export_routes.CarBodyGenerator") as MockGen:
