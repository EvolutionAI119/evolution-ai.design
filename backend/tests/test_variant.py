...
from app.database import init_db, SessionLocal, ModelVariant, ModelFile
...
class TestCreateVariant:
    ...
    def test_create_multiple_variants(self):
        model_id = _create_project_and_model()
        r1 = client.post("/api/v1/variants/", json={"model_id": model_id, "name": "变体A"})
        r2 = client.post("/api/v1/variants/", json={"model_id": model_id, "name": "变体B"})
...
class TestListVariants:
    ...
    def test_list_after_create(self):
        model_id = _create_project_and_model()
        client.post("/api/v1/variants/", json={"model_id": model_id, "name": "变体1"})
        client.post("/api/v1/variants/", json={"model_id": model_id, "name": "变体2"})
        resp = client.get(f"/api/v1/variants/{model_id}")
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 2
...
class TestVersionHistory:
    ...
    def test_history_with_variants(self):
        model_id = _create_project_and_model()
        client.post("/api/v1/variants/", json={"model_id": model_id, "name": "V1"})
        client.post("/api/v1/variants/", json={"model_id": model_id, "name": "V2"})
        resp = client.get(f"/api/v1/variants/{model_id}/history")
        assert resp.status_code == 200
        data = resp.json()
...
class TestCreateVariantWithFile:
    def test_create_variant_filepath_exists_reads_params(self):
        ...
        with patch.object(Path, "exists", return_value=True), \
             ...
             patch("app.routes.variant.CarBodyGenerator") as MockGen:
    ...
    def test_create_variant_filepath_exists_read_fails(self):
        ...
        with patch.object(Path, "exists", return_value=True), \
             ...
             patch("app.routes.variant.json.load", side_effect=Exception("read error")), \
             patch("app.routes.variant.CarBodyGenerator") as MockGen:
            MockGen.return_value.generate_complete_car.return_value = {
                "name": "完整车身", "components": [], "total_surfaces": 0,
                "parameters": {},
